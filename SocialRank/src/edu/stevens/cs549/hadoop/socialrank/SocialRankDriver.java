...
public class SocialRankDriver {
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 4) // If the number of input arguments is 4
		{
			...
			if (job.equals("init")) // Checks the first arguments for a match with each of the function names
			{
				init(args[1], args[2], Integer.parseInt(args[3])); // Sends data to the functions
...
//			} else if (job.equals("reciprocity")) {
//				reciprocity(args[1], args[2], Integer.parseInt(args[3]));
		...
		} else if (args.length == 5) // If number of input arguments is 5
		{
			...
			{
				diff(args[1], args[2], args[3], Integer.parseInt(args[4])); // Parses data to diff
			} else // In case the function name doesn't match up
			{
				System.err
						.println("Please check the name of the function you wish to call and try again");
			}
		} else if (args.length == 7) // If number of input arguments is 7 and first input is == "composite"; parses data to composite
		{
	...
	static void init(String input, String output, int reducers)
		...
		job.setJarByClass(SocialRankDriver.class); // Sets the Driver class
...
//	static void reciprocity(String input1, String output, int reducers)
//			throws Exception {
//		Job job = Job.getInstance(); // Creates a new job
//		job.setJarByClass(SocialRankDriver.class); // Sets Driver Class
//		job.setNumReduceTasks(reducers); // Sets number of reducers
//
//		FileInputFormat.addInputPath(job, new Path(input1)); // Adds input from
//																// one interim
//																// output
//		FileOutputFormat.setOutputPath(job, new Path("temprec")); // Adds a
//																	// temporary
//																	// output
//																	// folder
//																	// "temprec"
//
//		job.setMapperClass(ReciprocityMap1.class); // Sets Mapper and Reducer
//													// class for first job
//		job.setReducerClass(ReciprocityRed1.class);
//
//		job.setMapOutputKeyClass(Text.class); // Sets output classes for Mapper
//												// and Reducers for Job 1
//		job.setMapOutputValueClass(Text.class);
//
//		job.setOutputKeyClass(Text.class);
//		job.setOutputValueClass(Text.class);
//
//		System.out.println("First MapReduce Stage Initiated");
//
//		if (job.waitForCompletion(true)) // Once the first job completes
//		{
//			System.out.println("First MapReduce Stage Completed");
//			Job job1 = Job.getInstance(); // Creates a new second job
//			job1.setJarByClass(SocialRankDriver.class); // Sets driver class and
//														// number of reducers
//			job1.setNumReduceTasks(1); // Sets reducers to 1
//
//			FileInputFormat.addInputPath(job1, new Path("temprec")); // Adds the
//																		// temporary
//																		// directory
//																		// "tempdiff"
//																		// as
//																		// input
//			FileOutputFormat.setOutputPath(job1, new Path(output)); // Sets
//																	// output
//
//			job1.setMapperClass(ReciprocityMap2.class); // Sets Mapper and
//														// Reducer class for
//														// first job
//			job1.setReducerClass(ReciprocityRed2.class);
//
//			job1.setMapOutputKeyClass(Text.class); // Sets output class for
//													// second job
...
//
//			System.out.println("Second MapReduce Stage Initiated");
//
//			// If Job is completed, prints message, else prints an error
//			if (job1.waitForCompletion(true)) {
//				deleteDirectory("temprec"); // Deletes the temporary directory
//											// "temp"
//				System.out.println("Second MapReduce Stage Completed");
//			}
//
//			System.out.println();
//			System.out.println("Reciprocity Ratio = " + readDiffResult(output));
//		}
//
//	}
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		if (job.waitForCompletion(true)) // Once the first job completes
		{
			...
			Job job1 = Job.getInstance(); // Creates a new second job
			job1.setJarByClass(SocialRankDriver.class); // Sets driver class and number of reducers
			job1.setNumReduceTasks(reducers);

			FileInputFormat.addInputPath(job1, new Path("tempdiff")); // Adds the temporary directory "tempdiff" as input
	...
	public static void composite(String input, String output, String interim1,
		...
		System.out.println("Dhrupad Bhardwaj (dhrupadb)");
		int counter = 0;
		...
		{
			if (i % 2 == 0) {
				iter(interim1, interim2, reducers); // for every even iteration, interim1 is the input and
													// interim2 the output
			} else {
				iter(interim2, interim1, reducers); // For odd iterations it is swapped
			}
			if (i % 3 == 0) // for every 3 iterations calculates the difference
			{
				counter++;
				diff(interim1, interim2, "diffout", reducers); // as difference is an absolute value, order of input1
		...
		if (i % 2 == 1) // As i increments at the last step, for odd i, interim2 is the input directory
		{
	...
	// output file
	// From finish and sorts them in rank descending order.
	static void summarizeResult(String path) throws Exception {
		Path finpath = new Path(path); // Creates new Path
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(URI.create(path), conf);
		HashMap<Long, Double> values = new HashMap<Long, Double>(); // HasMap to store Node, Rank Pairs
		int size = 0;
		...
		if (fs.exists(finpath)) {
			...
			for (FileStatus file : ls) {
				if (file.getPath().getName().startsWith("part-r-00")) {
					...
					int i = 0;
					String diffcontent = "x"; // Initializes string to random
												// value
			...
			int j = 0;
			for (Map.Entry<Long, Double> entry : values.entrySet())// Iterates over set and stores values in arrays
			{
			...
			for (int i = 0; i < j - 1; i++) // Simple linear sort to order the nodes in reducing rank order
			{
				for (int k = i + 1; k < j; k++) {
					if (ranks[i] < ranks[k]) {
			...
			try {
				OutputStream os = fs.create(new Path(path + "/output.txt")); // Creates new file output.txt

				for (int i = 0; i < nodes.length; i++) {
					String out = nodes[i] + "\t" + ranks[i] + "\n"; // Print outs node rank pairs
					for (int k = 0; k < out.length(); k++) {
						char c = out.charAt(k);
						os.write(c); // Writes characters one by one to the output stream
					}
				}
